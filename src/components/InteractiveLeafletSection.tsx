...
const InteractiveLeafletSection = ({ filterLocations }: InteractiveLeafletSectionProps) => {
  ...
  return (
    <section className={`${filterLocations ? '' : 'py-16 bg-gradient-to-br from-slate-50 to-emerald-50'}`}>
      <div className={`${filterLocations ? '' : 'container mx-auto px-4'}`}>
        {!filterLocations && (
        ...
        <div className={`grid grid-cols-1 lg:grid-cols-3 gap-8 ${filterLocations ? 'h-full' : 'max-w-7xl mx-auto'}`}>
          ...
          {/* Details Container - Takes 1 column - only show when not filtered */}
          {!filterLocations && (
