...
const DestinationCard = ({ location, onClick }: DestinationCardProps) => {
  ...
  return (
    <div 
      className="mobile-card mobile-card-hover w-full h-[400px] sm:h-[400px] flex flex-col cursor-pointer group"
      onClick={handleClick}
    >
      ...
      <div className="mobile-card-content flex-1 flex flex-col pb-12 sm:pb-10">
        ...
        <div className="flex-1 min-h-[60px]">
          ...
          {location.bullet_points && location.bullet_points.length > 0 ? (
            <div className="bg-muted/30 rounded-lg p-3 mobile-spacing">
              {location.bullet_points.map((point, index) => (
                <div key={index} className="flex items-start space-x-2 mobile-text-sm text-foreground/80 mb-1.5 last:mb-0">
                  <div className="w-1 h-1 bg-primary rounded-full mt-2 flex-shrink-0" />
                  <span className="line-clamp-1 font-medium">{point}</span>
                </div>
              ))}
            </div>
