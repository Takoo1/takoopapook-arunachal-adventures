...
import { Menu, X, User, MapPin, ChevronDown, LogOut, MessageCircle } from 'lucide-react';
...
const Header = () => {
  ...
  const playAudio = () => {
    const audio = new Audio('https://mhhfdjtqzcbcixnpnaqp.supabase.co/storage/v1/object/sign/icons/TakooPapook_Audio.mp3?token=eyJraWQiOiJzdG9yYWdlLXVybC1zaWduaW5nLWtleV9mNzY4MTEyNS0zZjA1LTQ0YjMtOWMwOS1jOTg2ZTk1NWY0ZmQiLCJhbGciOiJIUzI1NiJ9.eyJ1cmwiOiJpY29ucy9UYWtvb1BhcG9va19BdWRpby5tcDMiLCJpYXQiOjE3NTU1MDExMzksImV4cCI6MTc4NzAzNzEzOX0.AZu5PUKduKwgXuP5hdiN9qX5itGgGtcXkNUA8rVJKx0');
    audio.play().catch(err => console.log('Audio play failed:', err));
  };

  const openWhatsApp = () => {
    const phoneNumber = '9365841168';
  ...
  return <header className={`fixed top-0 left-0 right-0 z-50 ${isNativeApp ? 'bg-transparent pt-8' : 'bg-white/95 backdrop-blur-md shadow-lg'}`}>
      <div className={`container mx-auto container-padding ${isNativeApp ? 'pt-4' : ''}`}>
        <div className="flex items-center justify-between h-14 sm:h-16 md:h-18">
          ...
          {/* WhatsApp Button - Only on Home page mobile */}
          {isMobile && isHomePage && (
            <button 
              onClick={openWhatsApp}
              className="flex items-center justify-center bg-transparent hover:scale-105 transition-transform duration-300"
            >
              <img 
