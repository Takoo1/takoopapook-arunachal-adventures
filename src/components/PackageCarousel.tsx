...
const PackageCarousel = () => {
  ...
  // Auto-scroll functionality - true infinite loop
  useEffect(() => {
  ...
  useEffect(() => {
    if (packages.length > 0 && currentIndex >= packages.length) {
    ...
    <section className="py-20 bg-gradient-to-br from-slate-50 to-emerald-50">
      <div className="container mx-auto px-4">
        ...
        <div className="relative overflow-hidden">
          <div 
            className={`flex ${currentIndex < packages.length ? 'transition-transform duration-700 ease-in-out' : ''}`}
            style={{ 
              transform: `translateX(-${currentIndex * (100 / 3)}%)`,
              width: `${(packages.length * 2) * (100 / 3)}%`
            }}
          >
            {/* Original cards */}
            {packages.map((pkg) => (
              <div key={pkg.id} className="w-full px-4" style={{ width: `${100 / (packages.length * 2)}%` }}>
            ...
            {packages.map((pkg) => (
              <div key={`duplicate-${pkg.id}`} className="w-full px-4" style={{ width: `${100 / (packages.length * 2)}%` }}>
                <div className="bg-white rounded-2xl shadow-lg hover:shadow-2xl transition-all duration-500 overflow-hidden group hover:-translate-y-2">
                  {/* Image */}
                  <div className="relative h-64 overflow-hidden">
                    <img 
          ...
          {packages.map((_, index) => (
            <button
              ...
              className={`w-3 h-3 rounded-full transition-all duration-300 ${
                (index === currentIndex || (currentIndex >= packages.length && index === currentIndex % packages.length))
                  ? 'bg-emerald-500 w-8' 
